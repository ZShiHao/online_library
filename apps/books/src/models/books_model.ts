...
},
{
  toJSON: {
    transform(doc, ret) {
      ret.id = ret._id;
      ret.cover=store.signatureUrl("imgs/cover/"+ret.cover,{expires:process.env.EXPIRES?Number(process.env.EXPIRES):undefined})
      delete ret._id;
      delete ret.__v;
    },
  },
...
const Book = mongoose.model<BookDoc, mongoose.PaginateModel<BookDoc>>("Book", bookSchema,'books');
